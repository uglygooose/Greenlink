...
def test_booking_completion_allows_checked_in_booking_and_reflects_in_tee_sheet(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="completion-admin@example.com")
    member = _create_user(db_session, email="completion-member@example.com")
    club = _create_club(db_session, name="Completion Club", slug="completion-club")
    ...
    before_slot = before_completion.json()["rows"][0]["slots"][0]
    assert before_slot["occupancy"]["occupied_player_count"] == 1
    assert before_slot["occupancy"]["confirmed_booking_count"] == 1
    assert before_slot["bookings"][0]["status"] == "checked_in"

    response = client.post(f"/api/golf/bookings/{booking.id}/complete", headers=headers)
    assert response.status_code == 200
    payload = response.json()

    assert payload["decision"] == "allowed"
    assert payload["transition_applied"] is True
    assert payload["booking"]["status"] == "completed"

    persisted = db_session.scalar(select(Booking).where(Booking.id == booking.id))
    assert persisted is not None
    assert persisted.status == BookingStatus.COMPLETED
    ...
    after_slot = after_completion.json()["rows"][0]["slots"][0]
    assert after_slot["occupancy"]["occupied_player_count"] == 0
    assert after_slot["occupancy"]["confirmed_booking_count"] == 0
    assert after_slot["party_summary"]["total_players"] == 0
    assert after_slot["bookings"] == []


def test_booking_completion_blocks_non_eligible_reserved_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="completion-block-admin@example.com")
    member = _create_user(db_session, email="completion-block-member@example.com")
    club = _create_club(db_session, name="Completion Block Club", slug="completion-block-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    assert payload["failures"][0]["code"] == "booking_status_not_completable"
    assert payload["failures"][0]["current_status"] == "reserved"
...
def test_booking_completion_is_idempotent_for_already_completed_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="completion-repeat-admin@example.com")
    member = _create_user(db_session, email="completion-repeat-member@example.com")
    club = _create_club(db_session, name="Completion Repeat Club", slug="completion-repeat-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    headers = _auth_headers(client, admin.email, str(club.id))
    first_response = client.post(f"/api/golf/bookings/{booking.id}/complete", headers=headers)
    assert first_response.status_code == 200
    assert first_response.json()["decision"] == "allowed"
    assert first_response.json()["transition_applied"] is True

    second_response = client.post(f"/api/golf/bookings/{booking.id}/complete", headers=headers)
