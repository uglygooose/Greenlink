...
def test_booking_no_show_allows_reserved_booking_and_reflects_in_tee_sheet(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="no-show-admin@example.com")
    member = _create_user(db_session, email="no-show-member@example.com")
    club = _create_club(db_session, name="No Show Club", slug="no-show-club")
    ...
    assert before_no_show.status_code == 200
    before_slot = before_no_show.json()["rows"][0]["slots"][0]
    ...
    response = client.post(f"/api/golf/bookings/{booking.id}/no-show", headers=headers)
    assert response.status_code == 200
    payload = response.json()

    assert payload["decision"] == "allowed"
    assert payload["transition_applied"] is True
    assert payload["booking"]["status"] == "no_show"
    ...
    assert after_no_show.status_code == 200
    after_slot = after_no_show.json()["rows"][0]["slots"][0]
...
def test_booking_no_show_blocks_non_eligible_checked_in_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="no-show-block-admin@example.com")
    member = _create_user(db_session, email="no-show-block-member@example.com")
    club = _create_club(db_session, name="No Show Block Club", slug="no-show-block-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    assert payload["failures"][0]["code"] == "booking_status_not_no_show_eligible"
...
def test_booking_no_show_is_idempotent_for_already_no_show_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="no-show-repeat-admin@example.com")
    member = _create_user(db_session, email="no-show-repeat-member@example.com")
    club = _create_club(db_session, name="No Show Repeat Club", slug="no-show-repeat-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    headers = _auth_headers(client, admin.email, str(club.id))
    first_response = client.post(f"/api/golf/bookings/{booking.id}/no-show", headers=headers)
    assert first_response.status_code == 200
    assert first_response.json()["decision"] == "allowed"
    assert first_response.json()["transition_applied"] is True

    second_response = client.post(f"/api/golf/bookings/{booking.id}/no-show", headers=headers)
