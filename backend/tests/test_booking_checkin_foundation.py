...
def test_booking_checkin_allows_reserved_booking_and_reflects_in_tee_sheet(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="checkin-admin@example.com")
    member = _create_user(db_session, email="checkin-member@example.com")
    club = _create_club(db_session, name="Check In Club", slug="check-in-club")
    ...
    assert before_checkin.status_code == 200
    before_slot = before_checkin.json()["rows"][0]["slots"][0]
    assert before_slot["occupancy"]["reserved_player_count"] == 1
    assert before_slot["occupancy"]["occupied_player_count"] == 0
    assert before_slot["bookings"][0]["status"] == "reserved"

    response = client.post(f"/api/golf/bookings/{booking.id}/check-in", headers=headers)
    ...
    assert after_checkin.status_code == 200
    after_slot = after_checkin.json()["rows"][0]["slots"][0]
    assert after_slot["occupancy"]["reserved_player_count"] == 0
    assert after_slot["occupancy"]["reserved_booking_count"] == 0
    assert after_slot["occupancy"]["occupied_player_count"] == 1
    assert after_slot["occupancy"]["confirmed_booking_count"] == 1
    assert after_slot["bookings"][0]["status"] == "checked_in"


def test_booking_checkin_blocks_non_eligible_cancelled_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="checkin-block-admin@example.com")
    member = _create_user(db_session, email="checkin-block-member@example.com")
    club = _create_club(db_session, name="Check In Block Club", slug="check-in-block-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    assert payload["failures"][0]["code"] == "booking_status_not_checkin_eligible"
...
def test_booking_checkin_is_idempotent_for_already_checked_in_booking(
    client: TestClient, db_session: Session
) -> None:
    admin = _create_user(db_session, email="checkin-repeat-admin@example.com")
    member = _create_user(db_session, email="checkin-repeat-member@example.com")
    club = _create_club(db_session, name="Check In Repeat Club", slug="check-in-repeat-club")
    _assign_membership(db_session, user=admin, club=club, role=ClubMembershipRole.CLUB_ADMIN)
    member_membership = _assign_membership(
        db_session,
        user=member,
    ...
    headers = _auth_headers(client, admin.email, str(club.id))
    first_response = client.post(f"/api/golf/bookings/{booking.id}/check-in", headers=headers)
    assert first_response.status_code == 200
    assert first_response.json()["decision"] == "allowed"
    assert first_response.json()["transition_applied"] is True

    second_response = client.post(f"/api/golf/bookings/{booking.id}/check-in", headers=headers)
