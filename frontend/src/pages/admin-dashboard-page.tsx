...
export function AdminDashboardPage(): JSX.Element {
  ...
  const arrearsAccounts = accounts.filter((account) => parseFloat(account.balance) < 0);
  ...
  const bookedSlots = allSlots.filter((slot) => slot.bookings.some((booking) => booking.status === "reserved" || booking.status === "checked_in")).length;
  ...
  return (
    <AdminShell title="Dashboard" searchPlaceholder="Search operations...">
      <AdminWorkspace
        description={`${selectedClubName}${timezone ? ` â€¢ ${timezone}` : ""}`}
        kpis={
                ...
                {outstandingSummaryQuery.isLoading ? (
                  ...
                  <>
                    ...
                    <span className="text-xs font-medium text-error">
                      {outstandingSummary?.accounts_in_arrears ?? 0} accounts
                    </span>
                  </>
                )}
              </div>
            ...
            <div className="rounded-xl bg-surface-container-lowest p-6 shadow-sm border-l-4 border-secondary">
              ...
              <div className="flex items-baseline gap-2">
                {directoryQuery.isLoading ? (
                  ...
                  <>
                    <span className="font-headline text-3xl font-extrabold text-on-surface">{memberCount ?? "â€”"}</span>
            ...
            <section>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="flex items-center gap-2 font-headline text-lg font-bold">
                  Operational Alerts
                  {((outstandingSummary?.accounts_in_arrears ?? 0) > 0 || teeWarnings.length > 0) && (
                    <span className="rounded-full bg-error-container px-2 py-0.5 text-[10px] uppercase tracking-tighter text-on-error-container">
                      {(outstandingSummary?.accounts_in_arrears ?? 0) + teeWarnings.length} Active
                    </span>
                  )}
                </h3>
              ...
              <div className="space-y-3">
                {(outstandingSummary?.accounts_in_arrears ?? 0) === 0 && teeWarnings.length === 0 && !accountsQuery.isLoading && (
                  <div className="flex items-center gap-4 rounded-xl bg-emerald-50 p-4">
                    ...
                    <p className="text-sm font-medium text-emerald-800">No operational alerts â€” all clear.</p>
                  </div>
                )}
                {arrearsAccounts.slice(0, 3).map((account) => (
                  <div className="flex items-center gap-4 rounded-xl bg-white p-4 shadow-sm" key={account.id}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-on-surface-variant">
                        {account.account_customer.account_code} â€” balance {formatAmountStr(account.balance)}
                      </p>
                    </div>
                ...
                {arrearsAccounts.length > 3 && (
                  <p className="pl-4 text-xs text-slate-400">+{arrearsAccounts.length - 3} more accounts in arrears</p>
                )}
              ...
              <div className="rounded-2xl bg-surface-container-low p-2">
                {journalQuery.isLoading && (
                  <div className="space-y-2 p-2">
                    {[1, 2, 3].map((item) => <div className="h-12 animate-pulse rounded-xl bg-slate-100" key={item} />)}
                  </div>
                )}
                    ...
                    return (
                      <div className="flex items-center gap-4 rounded-xl bg-surface-container-lowest p-4" key={entry.id}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-slate-400 capitalize">{entry.source} â€¢ {timeAgo(entry.created_at)}</p>
                        </div>
            ...
            <section className="rounded-2xl bg-surface-container-low p-6">
              ...
              <div className="space-y-3">
                {coursesQuery.isLoading ? (
                  <div className="space-y-3">
                    {[1, 2].map((item) => <div className="h-6 animate-pulse rounded bg-slate-100" key={item} />)}
                  </div>
