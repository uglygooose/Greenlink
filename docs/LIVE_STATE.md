...
Last regenerated: 2026-05-14 from commit `2c74afb`.
...
- Component primitives (`frontend/src/components/ui/`):
  ...
  - `Pill.tsx` (49 lines) — compact rounded label; `kind` variants (`open`, `booked`, `err`, etc.) mapped to the `--gl-*` palette.
  - `Kbd.tsx` (20 lines) — keyboard-key glyph styling for shortcut affordances.
  - `CardHead.tsx` (42 lines) — `eyebrow` / `title` / `right` composition for `Card` headers; consumed by Phase 7 + Phase 10 cards.
  - `TeeStateChip.tsx` (93 lines) — six-state chip (`open` / `booked` / `checkedin` / `atrisk` / `noshow` / `blocked`) for the tee-sheet legend and per-row state pill.
  - `Switch.tsx` (54 lines) — accessible toggle with `role="switch"` + `aria-checked`.
  - `Slider.tsx` (100 lines) — single-thumb range input with `aria-valuemin` / `aria-valuemax` / `aria-valuenow`.
  - `Segmented.tsx` (68 lines) — `role="radiogroup"` of inline buttons (`role="radio"` + `aria-checked` per segment); charcoal-bg / parchment-fg selected chrome; generic over string values. Consumed by `SlotIntervalToggle` and the finance export-granularity control.
  - `PricePopover.tsx` (394 lines) — additive price-breakdown popover with outside-click + esc dismiss; defers to higher-tier `aria-modal="true"` dialogs (Phase 10 Slice 5 + Slice 6 esc-priority contract; see DRIFT_LOG 2026-05-13 aria-modal entry).
  - `ShortcutHelpModal.tsx` (330 lines) — `role="dialog"` + `aria-modal="true"` keyboard-shortcut catalog; owns its own esc + backdrop dismiss (Phase 10 Slice 6).
  - 16 of the 18 primitives in `frontend/src/components/ui/` ship with dedicated `.test.tsx` files; `Avatar` and `HeroPlaceholder` are the two without explicit tests.
- Design-system components in feature folders:
  - `frontend/src/features/tee-sheet/components/SlotIntervalToggle.tsx` (28 lines) — 4-button segmented control over `{6, 8, 10, 12}` wrapping `Segmented`; mounted on the right of the tee-sheet date strip; selected button derives from `day.interval_minutes` (truth-from-server). Ships with a `.test.tsx` alongside (4 tests). Phase 10 Slice 11b.
...
- `/admin/tee-sheet` → `frontend/src/pages/admin-tee-sheet-page.tsx` (Phase 10 rebuild — Slices 1–11 + 11.5 + 11b; Slice 12 tournament mode deferred to Phase 11 per DRIFT_LOG 2026-05-14).
...
- `GET /api/golf/tee-sheet/day` — `get_tee_sheet_day` (`backend/app/api/routes/golf.py:290`). Optional `interval_minutes` query param (Phase 10 cleanup / Slice 11.5) accepts `{6, 8, 10, 12}` — request-scoped override of the day's slot bucketing; when omitted falls back to `ClubConfig.default_slot_interval_minutes`. Pydantic range guard (`ge=6, le=12`) returns 422 for out-of-range values; the route-body set check returns 400 for in-range non-allowed values (7, 9, 11). Override is not persisted.
...
- `POST /api/golf/tee-sheet/locks` → 201 — `acquire_tee_sheet_lock` (`backend/app/api/routes/golf.py:628`); 409 with a `TeeSheetLockConflictDetail` body when the slot is already held (Phase 10 Slice 8.5 optimistic-lock primitive; ApiError.body surfacing pattern per DRIFT_LOG 2026-05-14).
- `POST /api/golf/tee-sheet/locks/{lock_id}/renew` — `renew_tee_sheet_lock` (`backend/app/api/routes/golf.py:661`); refreshes a held lock's 60-second TTL.
- `DELETE /api/golf/tee-sheet/locks/{lock_id}` → 204 — `release_tee_sheet_lock` (`backend/app/api/routes/golf.py:685`); explicit release before TTL expiry.
- `GET /api/golf/tee-sheet/locks?course_id&date` — `list_tee_sheet_locks` (`backend/app/api/routes/golf.py:707`); active locks for a course+date, polled every 15s by the Slice 9b observer-side hook.
...
- Migration head: `202605140001` (`backend/alembic/versions/202605140001_add_tee_sheet_locks.py`).
...
- Most recent migration (`202605140001_add_tee_sheet_locks.py`) — adds the `tee_sheet_locks` table (Phase 10 Slice 8.5 optimistic-lock primitive). Columns: `id`, `club_id`, `course_id`, `slot_datetime`, `holder_user_id`, `expires_at`, `created_at`, `updated_at`. Unique constraint `uq_tee_sheet_locks_course_slot` on `(course_id, slot_datetime)`. Indexes on `club_id`, `course_id`, and `(course_id, slot_datetime, expires_at)`. Cascading FKs to `clubs`, `courses`, `users`. Service layer at `backend/app/services/tee_sheet_lock_service.py` with `LOCK_TTL_SECONDS = 60`. Locks are advisory — booking endpoints do NOT consult them.
- Previous migration (`202605130001_add_walk_in_booking_source.py`) — extends the native Postgres ENUM `bookingsource` with value `walk_in` via `ALTER TYPE ... ADD VALUE IF NOT EXISTS`. Empty downgrade (Postgres has no `DROP VALUE`). `BookingSource` enum now: `{admin, member_portal, staff, walk_in}` (`backend/app/models/enums.py:149-153`). Phase 10 Slice 7.5 prerequisite for the walk-in waitlist drop interaction (Slice 8a).
...
- Key endpoints: `GET /api/golf/tee-sheet/day` (with optional `interval_minutes` query param ∈ {6, 8, 10, 12} per Phase 10 Slice 11.5), `POST /api/golf/bookings`, `PATCH /api/golf/bookings/{booking_id}`, `POST /api/golf/bookings/{booking_id}/check-in`, `POST /api/golf/bookings/{booking_id}/complete`, `POST /api/golf/bookings/{booking_id}/no-show`, `POST /api/golf/bookings/{booking_id}/cancel`, `POST /api/golf/bookings/{booking_id}/move`, `POST /api/golf/tee-sheet/locks` + `POST /api/golf/tee-sheet/locks/{lock_id}/renew` + `DELETE /api/golf/tee-sheet/locks/{lock_id}` + `GET /api/golf/tee-sheet/locks?course_id&date` (Phase 10 Slice 8.5 optimistic-lock primitive).
- Notable surfaces: `frontend/src/pages/admin-tee-sheet-page.tsx` (Phase 10 rebuild — Slices 1–11 + 11.5 + 11b, mounted at `/admin/tee-sheet` per `frontend/src/routes/router.tsx:81`. Slice 12 / tournament-mode deferred to Phase 11 — no hole-addressable backend primitive, no tournament entity, no per-flight lock semantics; see DRIFT_LOG 2026-05-14.), `frontend/src/pages/admin-golf-tee-sheet-page.tsx` (3276 lines — legacy, mounted at `/admin/golf/tee-sheet`), `frontend/src/features/tee-sheet/sheet-shared.tsx` (1057 lines), `frontend/src/features/tee-sheet/booking-management-drawer.tsx` (622 lines), `frontend/src/features/tee-sheet/tee-sheet-swimlane-grid.tsx` (639 lines).
- Phase 10 modules live under `frontend/src/features/tee-sheet/`: `dnd/types.ts` + `dnd/use-drag-state.ts` (Slice 8a/8b — native HTML5 DnD primitives, no library; payload union supports `kind: "waitlist"` and `kind: "participant"`), `use-create-walkin-booking.ts` (Slice 8a walk-in booking mutation), `use-move-participant.ts` (Slice 8b cross-row participant move with categorised error surface), `use-participant-swap.ts` (Slice 8b two-call swap orchestrator with discriminated state machine + Retry/Restore handlers), `use-acquire-lock.ts` + `use-renew-lock.ts` + `use-release-lock.ts` + `use-selection-lock.ts` (Slice 9a — optimistic-lock holder-side orchestrator; state machine `idle | acquiring | held-by-me | held-by-other | releasing | error`; auto-renewal at 30s remaining; selection drives the lifecycle), `use-tee-sheet-locks.ts` + `lock-utils.ts` (Slice 9b — 15s polling of `GET /api/golf/tee-sheet/locks` plus the self-filtering Map derivation that drives the per-row badge), `use-checkin-booking.ts` + `use-mark-no-show.ts` (Slice 10 — booking lifecycle mutation hooks), `use-tee-sheet-shortcuts.ts` (Slice 10/11 — single document-level keydown listener wiring 22 shortcuts across Buckets A/B/C; `v` cycles density as of Slice 11; esc + ? continue on their pre-existing handlers), `use-density.ts` (Slice 11 — three-mode density cycle `compact | default | comfortable` persisted to `localStorage["gl.tee-sheet.density"]`; sets `data-density` on `document.documentElement`), `components/SlotIntervalToggle.tsx` (Slice 11b — 4-button segmented control over `{6, 8, 10, 12}` wrapping the Slice 1 `Segmented` primitive; mounted on the right of the date strip; selected button driven by `day.interval_minutes` — truth-from-server; interval change clears the row selection because slot times no longer align across intervals), `use-waitlist.ts` (Slice 7 empty stub — no `/api/golf/waitlist` endpoint), `components/PortfolioStrip.tsx` + `components/TeeRow.tsx` + `components/WaitlistCard.tsx` + `components/WaitlistRail.tsx` + `components/SelectionFooter.tsx` (Slice 9a — six-state lock line) + `components/PortfolioTile.tsx` + `components/PartialSwapPill.tsx` (Slices 1–9a chrome).
- Gaps:
  ...
  - Phase 10 Slice 8b accepts a v1 partial-state gap: no atomic-swap backend endpoint exists. Two-call sequential swap on a partial failure surfaces a `PartialSwapPill` for operator-driven Retry / Restore. See DRIFT_LOG 2026-05-14.
  - Phase 10 Slice 8b same-row reorder is client-rejected (no `participants.sort_order` on the read model). See DRIFT_LOG 2026-05-14.
  - Phase 10 closes after Slice 11 + the 11.5 / 11b cleanup pair (backend `interval_minutes` query param + frontend `SlotIntervalToggle`, both 2026-05-14). Slice 12 (tournament mode / shotgun view / Edit flights) deferred to Phase 11 — backend has no hole-addressing primitive (`StartLane` is binary HOLE_1/HOLE_10), no tournament/event/flight entity, no booking name field, and the lock primitive's `(course_id, slot_datetime)` unique constraint forces single-operator lock per shotgun start. Audit findings in DRIFT_LOG 2026-05-14.
...
- `frontend/src/pages/admin-tee-sheet-page.tsx` (new, Phase 10) ↔ `frontend/src/pages/admin-golf-tee-sheet-page.tsx` (legacy, 3276 lines). Phase 10 rebuilt the tee-sheet surface as the new page mounted at `/admin/tee-sheet`; the legacy page still resolves at `/admin/golf/tee-sheet` for direct URL access but is no longer linked from the sidebar — `frontend/src/components/admin-shell/AdminSidebar.tsx:35` points the "Tee sheet" nav item at the new route. The legacy page deletes when its remaining feature-folder consumers (`frontend/src/features/tee-sheet/sheet-shared.tsx`, `booking-management-drawer.tsx`, `tee-sheet-swimlane-grid.tsx`) migrate off it.

No new parallels emerged from the 9-series (which was backend-only). Phase 10 introduced the tee-sheet parallel above.
...
- **Price popover renders a single-line stub.** No backend additive price-breakdown endpoint exists; the frontend synthesises one `kind: "base"` line per booking from the existing `fee_label` + `fee_amount` shape. Evidence: `frontend/src/features/tee-sheet/use-price-breakdown.ts:29` (`// FROZEN — backend gap …`). See DRIFT_LOG 2026-05-13 (Slice 5).
- **Tee-row state mapping fold-down.** `checkedin` and `noshow` render as `booked` because `TeeSheetSlotView.display_status` carries only 5 of the 6 Phase 8 row states. Evidence: `frontend/src/features/tee-sheet/components/TeeRow.tsx:76` (`// FROZEN — backend gap …`). See DRIFT_LOG 2026-05-13 (Slice 2).
- **Per-cell channel dot omitted.** `TeeSheetBookingSummary` has no `source` / `channel` field; Slice 7.5 added `WALK_IN` to the `BookingSource` enum on 2026-05-13, but the value is still not surfaced via the day response. Evidence: `frontend/src/features/tee-sheet/components/TeeRow.tsx:323` (`{/* FROZEN — backend gap … */}`). See DRIFT_LOG 2026-05-13 (Slice 2).
- **Per-row audit clock omitted.** `TeeSheetSlotView` has no `has_audit_events` boolean; the Phase 9B emission infrastructure records events but no read-model summary is published per-slot. Evidence: `frontend/src/features/tee-sheet/components/TeeRow.tsx:736` (`{/* FROZEN — backend gap … */}`). See DRIFT_LOG 2026-05-13 (Slice 2).
- **Walk-in waitlist hook ships against an empty stub.** No `Waitlist` model, no `/api/golf/waitlist` endpoint, no suggestion engine; `useWaitlist` returns empty data unconditionally so the rail chrome and drop targets still render. Evidence: `frontend/src/features/tee-sheet/use-waitlist.ts:6` (`// FROZEN — backend gap …`) + `:51` (FROZEN swap-point inside `synthesizeStubWaitlist()`). See DRIFT_LOG 2026-05-13 (Slice 7).
- **Waitlist suggestion strip stays hidden until backend ships.** `WaitlistCard` conditionally renders the suggestion strip when `entry.suggestion` is truthy; the engine that produces "fits 06:46 · 2 slots" semantics is a product decision (capacity policy, party-mix rules, time-of-day preferences) deferred. Evidence: `frontend/src/features/tee-sheet/components/WaitlistCard.tsx:136` (`/* FROZEN — backend gap … */`). See DRIFT_LOG 2026-05-13 (Slice 7).
- **Waitlist rail footer running-total aggregated locally.** Sum of backend-provided `fee_amount` values, same idiom as Slices 2/3. Evidence: `frontend/src/features/tee-sheet/components/WaitlistRail.tsx:53` (`// FROZEN — backend gap …`). See DRIFT_LOG 2026-05-13 (Slice 7).
