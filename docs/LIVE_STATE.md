...
- Phase 10 modules live under `frontend/src/features/tee-sheet/`: `dnd/types.ts` + `dnd/use-drag-state.ts` (Slice 8a/8b — native HTML5 DnD primitives, no library; payload union supports `kind: "waitlist"` and `kind: "participant"`), `use-create-walkin-booking.ts` (Slice 8a walk-in booking mutation), `use-move-participant.ts` (Slice 8b cross-row participant move with categorised error surface), `use-participant-swap.ts` (Slice 8b two-call swap orchestrator with discriminated state machine + Retry/Restore handlers), `use-waitlist.ts` (Slice 7 empty stub — no `/api/golf/waitlist` endpoint), `components/PortfolioStrip.tsx` + `components/TeeRow.tsx` + `components/WaitlistCard.tsx` + `components/WaitlistRail.tsx` + `components/SelectionFooter.tsx` + `components/PortfolioTile.tsx` + `components/PartialSwapPill.tsx` (Slices 1–8b chrome).
- Gaps:
