...
- **Claim**: Phase 10's published slice plan included Slice 12 (tournament mode — 6×3 shotgun grid, tournament banner, Edit flights affordance, "the keyboard model persists" in tournament view) as the final slice of the tee-sheet rebuild.
- **Reality**: Backend cannot support a hole-addressable shotgun grid today. The data model, the response shape, and the lock primitive all assume the time-grid orientation; no tournament/event/flight entity exists.
  - **No tournament/event/flight entity.** `grep -rln "shotgun\|tournament\|\bflight\b" backend/app/` returns 0 hits across models, schemas, services, routes. The only competition-adjacent surface is the `competition_controlled: bool` flag on `TeeSheetSlotState` (`backend/app/models/tee_sheet_slot_state.py:64-66`), read by `booking_move_service.py:229-241` to reject moves into competition slots but never set by any application code path (only by `seed_demo_golf_data.py:684` for dev data).
  - **`StartLane` is binary.** `backend/app/models/enums.py:205-207` defines `class StartLane(StrEnum): HOLE_1 = "hole_1"; HOLE_10 = "hole_10"`. Zero references to `hole_2..hole_9` or `hole_11..hole_18` anywhere in `backend/app/`. The 6×3 shotgun grid wants 18 hole-keyed tiles; the backend cannot address holes beyond first-tee and tenth-tee.
  - **`TeeSheetDayResponse.rows` is per-lane, not per-hole.** `_load_row_scopes` (`backend/app/services/tee_sheet_service.py:221-238`) returns `(Tee × StartLane)` tuples, defaulting to `[HOLE_1, HOLE_10]`. A single-tee course returns ≤ 2 rows; the response cannot model 18 hole rows.
  - **No booking name / flight name field.** `BookingUpdateRequest` (`backend/app/schemas/bookings.py:179-186`) accepts `participants`, `holes`, `applies_to`, `reference_datetime`, `cart_flag`, `caddie_flag`. The `Booking` model's only string label is `fee_label: Mapped[str | None] = mapped_column(String(120), nullable=True)` (`backend/app/models/booking.py:83`). Phase 8's "Flight A — Smith, Jones, …" cannot be persisted.
  - **Lock primitive collides with shotgun semantics.** `TeeSheetLock` `UniqueConstraint("course_id", "slot_datetime")` (`backend/app/models/tee_sheet_lock.py:33-38`) permits one lock per slot. In a shotgun, all flights share the same `slot_datetime` → only one operator can edit the entire shotgun at a time. This contradicts Phase 8's implicit per-flight concurrency model.
  - **AdminSidebar already marks "Competitions" as Phase 11.** `frontend/src/components/admin-shell/AdminSidebar.tsx:55` (`{ icon: "emoji_events", label: "Competitions", comingPhase: "Phase 11" }`) — a pre-existing frontend marker that the competitions feature lives in the next phase, not Phase 10.
  - **Phase 8 design gaps not resolvable from the repo.** No "Edit flights" surface design extracted; no shotgun-mode keyboard model; no banner copy/composition spec. Claude Design engagement would be required regardless of backend work.
- **Evidence**: Full audit report in conversation transcript (Phase 10 Slice 12 reconnaissance, 2026-05-14). Quotes verbatim from the files referenced above.
- **Resolution**: Slice 12 deferred from Phase 10 to Phase 11 by user decision after audit (Path 1 of three audit-recommended paths). Phase 10 ships through Slice 11 (density toggle). Phase 11 will own tournament mode end-to-end — including the prerequisite backend work (hole-addressing primitive: light = `StartLane` enum extension, medium = `bookings.hole_number` column, heavy = `Tournament` entity; lock-key extension to include tee/lane), the Claude Design engagement (Edit flights surface, shotgun keyboard model, banner composition), and the frontend slice that consumes the resulting shape. The Slice 10 `⇧T` keystroke handler remains a forward-reference stub announcing "Tournament mode arrives in Slice 12" — its message will need updating once Phase 11 owns the slice (the Slice 10 chip-comment forward-reference at `frontend/src/features/tee-sheet/use-tee-sheet-shortcuts.ts:131-134` is the single touchpoint).
- **Status**: Deferred to Phase 11. Phase 10 closes after Slice 11.
---
...
- **Status**: **Resolved**: Slice 11.5 (backend) + Slice 11b (frontend, 2026-05-14) shipped the `interval_minutes` query param and the `SlotIntervalToggle` UI. Allowed values: 6, 8, 10, 12. Override is page-state only (no URL or localStorage persistence in v1). Backend: route accepts `interval_minutes` with two-layer validation — Pydantic `ge=6, le=12` rejects out-of-range (5/15 → 422); the route's set check rejects in-range non-allowed values (7/9/11 → 400). `TeeSheetDayQuery.interval_minutes_override` threads through to `TeeSheetService._resolve_interval_minutes`, which falls back to `ClubConfig.default_slot_interval_minutes` when null. Frontend: `fetchTeeSheetDay` + `useTeeSheetDayQuery` accept optional `intervalMinutes`; `teeSheetKeys.day` collapses null/undefined to the `"default"` sentinel so PortfolioStrip's per-course summary keys (no override) share cache with the page's main day query when no override is active. `SlotIntervalToggle` mounts on the right of the date strip; selected button is driven by `day.interval_minutes` (truth-from-server), not the local override state. Interval change clears the row selection.
---
