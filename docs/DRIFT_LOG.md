...
- **Status**: **Resolved by Phase 10 cleanup / Slice 11.5 (2026-05-14).** Backend route now accepts `interval_minutes` as an optional query param (allowed set: `{6, 8, 10, 12}`). `TeeSheetDayQuery` gained `interval_minutes_override`; `TeeSheetService._resolve_interval_minutes` falls back to `ClubConfig.default_slot_interval_minutes` when the override is None. Two-layer validation: Pydantic `ge=6, le=12` rejects out-of-range (5/15 → 422); the route's set check rejects in-range non-allowed values (7/9/11 → 400). Override is request-scoped; not persisted. Slice 11b frontend slice (wires the `SlotIntervalToggle` to the now-available query param) is the remaining follow-up.
---
