...
def _normalize_communication_value(raw: str | None, allowed: set[str], field_name: str) -> str:
    value = str(raw or "").strip().lower()
...
def _serialize_club_communication(row: ClubCommunication) -> dict:
    return {
...
def list_club_communications(
    ...
    rows = (
        q.order_by(ClubCommunication.pinned.desc(), ClubCommunication.published_at.desc(), ClubCommunication.created_at.desc())
        .limit(int(limit))
        .all()
    )
    return {
        "count": len(rows),
        "communications": [_serialize_club_communication(row) for row in rows],
    }
...
def create_club_communication(
    ...
    row = ClubCommunication(
        club_id=int(club_id),
        kind=_normalize_communication_value(payload.kind, {"news", "announcement", "message"}, "kind"),
        audience=_normalize_communication_value(payload.audience, {"members", "staff", "all"}, "audience"),
        status=_normalize_communication_value(payload.status, {"draft", "published", "archived"}, "status"),
        ...
        created_by_user_id=int(getattr(admin, "id", 0) or 0) or None,
    ...
    db.commit()
    db.refresh(row)
    return _serialize_club_communication(row)
...
def update_club_communication(
    ...
    row.kind = _normalize_communication_value(payload.kind, {"news", "announcement", "message"}, "kind")
    row.audience = _normalize_communication_value(payload.audience, {"members", "staff", "all"}, "audience")
    row.status = _normalize_communication_value(payload.status, {"draft", "published", "archived"}, "status")
